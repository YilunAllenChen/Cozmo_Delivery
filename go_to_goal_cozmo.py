...
from localization_gui import GUIWindow
...
localization_gui = GUIWindow(grid, show_camera=True)
...
async def localization(robot: cozmo.robot.Robot):
    global flag_odom_init, last_pose
    global grid, localization_gui, pf
    ...
    while not converged:
        ...
        # Update localization_gui
        localization_gui.show_particles(pf.particles)
        localization_gui.show_mean(*m)
        localization_gui.updated.set()
        localization_gui.show_camera_image(annotated_image)
    ...
    await robot.turn_in_place(cozmo.util.degrees(-theta+goal_h),
                                speed=cozmo.util.Angle(degrees=60)).wait_for_completed()
